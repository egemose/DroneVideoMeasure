...
from dvm.drone.drone_log_data import get_video_ranges
import logging
...
def get_log_plot(log_data):
    ...
    grid = gridplot(
        [height_plot, yaw_plot, pitch_plot, roll_plot],
        ncols=1,
...
def get_log_plot_with_video(log_data, video_start, video_duration, video_frames):
    ...
    video_box = BoxAnnotation(
        left=video_start, right=video_end_time, top=1, fill_color="#00cc00"
    )
    video_position_line1 = Span(
        location=video_start, dimension="height", line_color="red"
    )
    height_plot.add_layout(video_box)
    height_plot.add_layout(video_position_line1)
    video_box = BoxAnnotation(
        left=video_start,
        right=video_end_time,
        top=yaw_plot.y_range.end,
        bottom=yaw_plot.y_range.start,
        fill_color="#00cc00",
    )
    video_position_line2 = Span(
        location=video_start, dimension="height", line_color="red"
    )
    yaw_plot.add_layout(video_box)
    yaw_plot.add_layout(video_position_line2)
    video_box = BoxAnnotation(
        left=video_start, right=video_end_time, bottom=5, fill_color="#00cc00"
    )
    video_position_line3 = Span(
        location=video_start, dimension="height", line_color="red"
    )
    pitch_plot.add_layout(video_box)
    pitch_plot.add_layout(video_position_line3)
    video_box = BoxAnnotation(
        left=video_start, right=video_end_time, top=1, bottom=-1, fill_color="#00cc00"
    )
    video_position_line4 = Span(
        location=video_start, dimension="height", line_color="red"
    )
    roll_plot.add_layout(video_box)
    roll_plot.add_layout(video_position_line4)
    args = dict(
        span1=video_position_line1,
...
def _get_log_plots(log_data):
    ...
    height_plot.xaxis.formatter = DatetimeTickFormatter(
        minsec="%H:%M:%S", minutes="%H:%M:%S", hourmin="%H:%M:%S"
    )
    ...
    yaw_plot.xaxis.formatter = DatetimeTickFormatter(
        minsec="%H:%M:%S", minutes="%H:%M:%S", hourmin="%H:%M:%S"
    )
    yaw_plot.yaxis.axis_label = "Degrees"
    yaw_plot.line(time_stamp, yaw)
    pitch_plot = figure(
        title="Pitch", width=700, height=500, x_range=height_plot.x_range
    )
    pitch_plot.xaxis.formatter = DatetimeTickFormatter(
        minsec="%H:%M:%S", minutes="%H:%M:%S", hourmin="%H:%M:%S"
    )
    ...
    roll_plot.xaxis.formatter = DatetimeTickFormatter(
        minsec="%H:%M:%S", minutes="%H:%M:%S", hourmin="%H:%M:%S"
    )
    roll_plot.yaxis.axis_label = "Degrees"
    roll_plot.line(time_stamp, roll)
    for video_range in get_video_ranges(is_video, time_stamp):
        video_box1 = BoxAnnotation(
            left=video_range[0], right=video_range[1], fill_color="#cccccc"
        )
        video_box2 = BoxAnnotation(
            left=video_range[0], right=video_range[1], fill_color="#cccccc"
        )
        video_box3 = BoxAnnotation(
            left=video_range[0], right=video_range[1], fill_color="#cccccc"
        )
        video_box4 = BoxAnnotation(
            left=video_range[0], right=video_range[1], fill_color="#cccccc"
        )
